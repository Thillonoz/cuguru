...
bool IsMoveValid(Cell grid[COLS][ROWS], int cx, int cy, int val)
{
    ...
    for (int x = 0; x < COLS; x++)
    {
        for (int y = 0; y < ROWS; y++)
        {
            if (x == cx && y == cy)
                continue;
            if (grid[x][y].region_id == target_region && grid[x][y].value == val)
                return false;
        }
    }
    return true;
...
bool SolveGrid(Cell grid[COLS][ROWS], int cx, int cy, int *guess_count)
{
    ...
    if (grid[cx][cy].value != 0)
        return SolveGrid(grid, nx, ny, guess_count);
    ...
    for (int i = 0; i < r_size; i++)
    {
        ...
        if (IsMoveValid(grid, cx, cy, v))
        {
            grid[cx][cy].value = v;
            (*guess_count)++;
            if (SolveGrid(grid, nx, ny, guess_count))
                return true;
            grid[cx][cy].value = 0;
        }
    }
...
void *GeneratorWorker(void *arg)
{
    ...
    while (!atomic_load(&puzzle_found))
    {
        ...
        int solve_guesses = 0;
        if (!SolveGrid(local_grid, 0, 0, &solve_guesses))
            continue;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            int temp_x = coords[i][0];
            int temp_y = coords[i][1];
            coords[i][0] = coords[swap_idx][0];
            coords[i][1] = coords[swap_idx][1];
            coords[swap_idx][0] = temp_x;
            coords[swap_idx][1] = temp_y;
        }
        ...
        for (int i = 0; i < count; i++)
        {
            if (erased_count >= target_erasures)
                break;
        ...
        if (thread_stuck || atomic_load(&puzzle_found))
            continue;
    ...
    while (!WindowShouldClose())
    {
        ...
        else if (!is_game_won)
        {
            ...
            if (selected_x != -1 && selected_y != -1 && !grid[selected_x][selected_y].is_given)
            {
                ...
                {
                    if ((key - KEY_ZERO) == grid[selected_x][selected_y].value)
                    {
                ...
                else if (key >= KEY_KP_1 && key <= KEY_KP_5)
                {
                    if ((key - KEY_KP_0) == grid[selected_x][selected_y].value)
                    {
                        grid[selected_x][selected_y].value = 0;
                    }
                    else
                    {
                        grid[selected_x][selected_y].value = key - KEY_KP_0;
                    }
                }
        ...
        for (int x = 0; x < COLS; x++)
        {
            for (int y = 0; y < ROWS; y++)
            {
                ...
                if (grid[x][y].value > 0 && !is_generating)
                {
                    ...
                    if (!isValid)
                        textColor = RED;
                    if (grid[x][y].is_given)
                        textColor = ColorAlpha(BLACK, 0.6f);
        ...
        for (int x = 0; x < COLS; x++)
        {
            for (int y = 0; y < ROWS; y++)
            {
                ...
                if (x == 0)
                    DrawLineEx((Vector2){0, y * CELL_SIZE}, (Vector2){0, (y + 1) * CELL_SIZE}, 4.0f, BLACK);
                if (y == 0)
                    DrawLineEx((Vector2){x * CELL_SIZE, 0}, (Vector2){(x + 1) * CELL_SIZE, 0}, 4.0f, BLACK);
            }
        }
        ...
        if (!is_generating)
        {
            if (DrawButton(btnNew, "New Puzzle", LIGHTGRAY, false) || IsKeyPressed(KEY_N))
            {
                is_generating = true;
                atomic_store(&puzzle_found, false);
                atomic_store(&total_attempts, 0);
            ...
            if (DrawButton(btnReset, "Reset Grid", LIGHTGRAY, false) || IsKeyPressed(KEY_R))
            {
            ...
            if (!is_game_won)
            {
                if (DrawButton(btnHint, "Get a Hint", SKYBLUE, false) || IsKeyPressed(KEY_H))
                {
            ...
            {
                Rectangle btn = (diff == 0) ? btnEasy : (diff == 1) ? btnMed
                                                                    : btnHard;
                const char *txt = (diff == 0) ? "Easy" : (diff == 1) ? "Medium"
                                                                     : "Hard";
                Color col = (diff == 0) ? LIME : (diff == 1) ? ORANGE
                                                             : RED;
